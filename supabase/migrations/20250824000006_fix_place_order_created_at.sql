-- Fix place_order function - resolve created_at ambiguity
-- Migration: 20250824000006_fix_place_order_created_at.sql

-- Enhanced place_order function with fixed created_at reference
...
BEGIN
    ...
    -- Create order (fix created_at ambiguity by using explicit column references)
    INSERT INTO order_book (
        ...
        'pending'
    ) RETURNING id, order_book.created_at INTO v_order_id, v_order_created_at;
    
    -- Update fund reservation with order_id
    ...
    SELECT order_book.status INTO v_order_status
    FROM order_book
    WHERE id = v_order_id;
    ...
    RETURN QUERY SELECT
        ...
        v_order_created_at,
        CASE 
            WHEN v_order_status = 'filled' THEN 'Order executed completely'
            WHEN v_order_status = 'partially_filled' THEN 'Order executed partially'

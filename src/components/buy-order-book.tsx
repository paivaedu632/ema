...
// Order Book Buy Component - Professional Trading Interface
// Replaces legacy marketplace-based buy component
...
export function BuyOrderBook() {
  ...
  // Get available balance for the quote currency (what user needs to spend)
  ...
  const getEstimatedCost = (): number => {
    ...
    if (watchedOrderType === 'market') {
      // For market orders, use best ask price
      return orderBookData.best_ask ? quantity * orderBookData.best_ask : 0
    } else {
  ...
  // Submit order
  const onSubmit = async (data: BuyOrderForm) => {
    setIsLoading(true)
  ...
  // Render form step
  const renderForm = () => {
    const availableBalance = getAvailableBalance(watchedQuoteCurrency)
    const estimatedCost = getEstimatedCost()
    const amountValidation = watchedQuantity ? validateAmount(watchedQuantity, watchedBaseCurrency) : null
    const hasInsufficientBalance = estimatedCost > availableBalance
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4 mb-6">
          ...
          <h1 className="text-2xl font-bold">Comprar Moeda</h1>
        </div>
        ...
        <Card>
          ...
          <CardContent className="space-y-4">
            <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                  ...
                  <CardContent className="pt-4">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      <div className="flex justify-between">
                        ...
                        <span>{estimatedCost.toFixed(2)} {watchedQuoteCurrency}</span>
                      </div>
                      <div className="flex justify-between">
                        <span>Saldo Disponível:</span>
                        <span>{availableBalance.toFixed(2)} {watchedQuoteCurrency}</span>
                      </div>
              ...
              >
                {isLoading ? 'Processando...' : 'Criar Ordem de Compra'}
              </Button>
            </form>
          </CardContent>
        </Card>
  ...
  const renderSuccess = () => (
    <div className="space-y-6">
      <div className="text-center">
        ...
        <p className="text-gray-600">Sua ordem de compra foi criada e está sendo processada.</p>
      </div>
